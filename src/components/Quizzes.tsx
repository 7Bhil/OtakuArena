    ...
    <div className="pt-32 pb-20 max-w-[1600px] mx-auto w-full px-6 lg:px-10 flex flex-col lg:flex-row gap-8">
      ...
      <aside className="w-full lg:w-80 flex flex-col gap-6">
        <div className="bg-primary/5 rounded-xl p-6 border border-primary/10 shadow-sm">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              <h3 className="text-xs uppercase tracking-widest text-slate-400 font-bold">Current Rank</h3>
              <p className="text-xl font-bold text-white">S-Class Elite</p>
            </div>
          </div>
          <div className="space-y-3">
            <div className="flex justify-between text-sm">
              <span className="text-slate-400">Progress to Legend</span>
              <span className="text-primary font-bold">75%</span>
            </div>
            <div className="w-full h-3 bg-slate-800 rounded-full overflow-hidden">
              <div className="h-full bg-primary" style={{ width: '75%' }}></div>
            </div>
            <p className="text-[11px] text-slate-400 text-center font-medium">1,250 XP remaining for Legend status</p>
          </div>
        ...
        <div className="bg-primary/5 rounded-xl p-6 border border-primary/10 shadow-sm">
          <h3 className="text-white font-bold mb-4 flex items-center gap-2">
            <Clock className="size-4 text-primary" /> Recent Quizzes
          </h3>
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <p className="text-sm font-bold text-slate-200 line-clamp-1">Naruto: Chuunin Exams</p>
                <p className="text-xs text-slate-500">95% Score • +450 OP</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <p className="text-sm font-bold text-slate-200 line-clamp-1">90s Retro Classics</p>
                <p className="text-xs text-slate-500">62% Score • +120 OP</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="flex-1 flex flex-col gap-6">
        <div className="relative overflow-hidden rounded-2xl bg-primary/10 border border-primary/20 p-8">
          <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
            <div className="text-center md:text-left">
              <h1 className="text-3xl md:text-4xl font-black text-white leading-tight mb-2">Quiz Hub</h1>
              <p className="text-slate-400 text-lg max-w-lg">Test your anime knowledge, climb the global rankings and earn Otaku Points (OP) for exclusive rewards.</p>
              <div className="mt-6 flex flex-wrap gap-3 justify-center md:justify-start">
                <button className="bg-primary text-white px-6 py-3 rounded-xl font-bold flex items-center gap-2 hover:scale-105 transition-transform shadow-lg shadow-primary/20">
                  <Zap className="size-5" /> Random Challenge
                </button>
        ...
        <div className="flex items-center gap-3 overflow-x-auto pb-2 no-scrollbar whitespace-nowrap">
          <button className="px-5 py-2.5 rounded-xl bg-primary text-white font-bold text-sm shadow-md shadow-primary/20">All Quizzes</button>
          {['Shonen Master', '90s Retro', 'Studio Ghibli', 'Isekai Tales', 'Seinen Lore'].map(cat => (
            ...
            >
              ...
              <div className="p-5 flex flex-col flex-1">
                ...
                <div className="mt-auto pt-5 flex items-center justify-between">
                  ...
                  <button className="bg-primary/10 text-primary hover:bg-primary hover:text-white px-4 py-2 rounded-lg text-sm font-bold transition-all">Start Quiz</button>
                </div>
              </div>
            </motion.div>
          ))}
        ...
        <div className="mt-8 flex justify-center">
          <button className="px-8 py-3 bg-primary/5 border border-primary/10 rounded-xl text-white font-bold hover:bg-primary/10 transition-all flex items-center gap-2">
            <ChevronDown className="size-5" /> Load More Quizzes
          </button>
        </div>
      </section>
    </div>
